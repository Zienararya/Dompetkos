...
import 'package:dompetkos/models/anggaran.dart';
//pubspec.yml
...
//create databases
  Future<int> insert(Anggaran object) async {
    Database db = await this.database;
...
//update databases
  Future<int> update(Anggaran object) async {
    Database db = await this.database;
...
//delete databases
  ...
  Future<List<Anggaran>> getContactList() async {
    var contactMapList = await select();
    int count = contactMapList.length;
    List<Anggaran> contactList = List<Anggaran>();
    for (int i = 0; i < count; i++) {
      contactList.add(Anggaran.fromMap(contactMapList[i]));
    }
    return contactList;
  }
}
